...
available_track_files = glob.glob("BreadCentric_Speedway.npy")
...
waypoints = np.load("BreadCentric_Speedway.npy")
print(waypoints)
...
def plot_coords(ax, ob):                                                        
    x, y = np.array(ob.coords[:]).T  # 将.xy替换为.coords[:]
    ...
    for i in range(0,len(new_line)):
        ...
        for j in range(0,XI_ITERATIONS):
            ...
            if p_ci < target_ci:
                ...
                # just abandon the refinement at this poinnt.  As adjacent
